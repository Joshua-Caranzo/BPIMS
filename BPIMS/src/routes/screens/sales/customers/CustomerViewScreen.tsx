...
const CustomerViewScreen = React.memo(({ route }: Props) => {
            ...
            if (customerId != 0 && customerId != null) {
                ...
                if (response) {
                    ...
                    if (response.data.customer.fileName)
                        image = await getCustomerImage(response.data.customer.fileName)
                    setFileUrl(image)
                }
                if (responeLoyalty) {
    ...
    return (
        <View className="flex flex-1">
            ...
            <TitleHeaderComponent showTrash={customer && customer.id != 0} onTrashPress={() => customer && removeCustomer(customer?.id)} isParent={false} userName={user.name} title={customer && customer.id != 0 ? customer.name : "New Customer"} onPress={() => navigation.navigate("Customer")}></TitleHeaderComponent>
